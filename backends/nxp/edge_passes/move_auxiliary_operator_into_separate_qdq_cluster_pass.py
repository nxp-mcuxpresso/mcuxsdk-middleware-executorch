...
class MoveLeadingAuxiliaryOperatorIntoSeparateQDQClusterPass(NXPEdgePass):
    ...
    def run(self, graph_module: torch.fx.GraphModule) -> PassResult:
        ...
        for aux_node in graph_module.graph.nodes:
            ...
            if any(node_ not in cluster for node_ in [dequantize_node, aux_node, main_cluster_node]):
                continue
...
class MoveTrailingAuxiliaryOperatorIntoSeparateQDQClusterPass(NXPEdgePass):
    """
                                                                  │
                                                            ┌─────▼──────┐
                      │                                     │ dequantize │
    ...
    def run(self, graph_module: torch.fx.GraphModule) -> PassResult:
        ...
        for aux_node in graph_module.graph.nodes:
            ...
            # Make sure the nodes are part of the same QDQ cluster.
            cluster = QDQClusterRecognizer().get_qdq_cluster(main_cluster_node)
            if any(node_ not in cluster for node_ in [quantize_node, aux_node, main_cluster_node]):
                continue
