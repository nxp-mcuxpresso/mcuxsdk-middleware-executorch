...
from executorch.backends.nxp.backend.ir.tflite_generator.builtin_options import split_options, concatenation_options, \
    conv_2d_options
...
def _get_IO_channels(node: Node | tflite_model.Operator) -> (int, int):
    if isinstance(node, Node):
        input_channels = node.args[0].meta['val'].shape[1]  # Channels of the main input.
        output_channels = node.args[1].meta['val'].shape[0]  # Output channels of the weights.
    else:
...
def group_conv_convertible_into_multiple_convolutions(node: Node | tflite_model.Operator, group: int) -> bool:
    if group == 1:
        return False
...
class ConvConversionResult:
    ...
    def __init__(self, input_tensor: tflite_model.Tensor, weight_tensor: tflite_model.Tensor,
                 bias_tensor: tflite_model.Tensor, output_tensor: tflite_model.Tensor,
...
ConvOpFactory = Callable[
    [ConvParameters, tflite_model.Tensor, tflite_model.Tensor, tflite_model.Tensor, tflite_model.Tensor, ModelBuilder,
     ConvBuiltinOptions], OpsList
]
ConvConversionFn = Callable[[tflite_model.Operator, ConvParameters], ConvConversionResult]
...
class _InputTensorsSplitter:
    ...
    def __init__(self, input_tensor: tflite_model.Tensor, weight_tensor: tflite_model.Tensor,
                 bias_tensor: tflite_model.Tensor, groups: int,
    ...
    def _create_split_op(self, builder, groups, input_tensor, axis):
        ...
        input_split_op = tflite_model.Operator(builtin_options=split_options.Split(groups))
...
def build_input_tensor_padding(t_op, conv_params: ConvParameters, builder, input_idx=0
    ...
    if explicit_padding is not None:
        ...
        return tfl_padding, builder.create_pad_operator_before(t_op, input_idx, explicit_padding)

    return tfl_padding, None


def conv_op_factory(conv_params: ConvParameters, input_tensor: tflite_model.Tensor,
...
def create_separated_convolutions_based_on_group(t_op: tflite_model.Operator, conv_params: ConvParameters,
                                                 builder: ModelBuilder,
                                                 conv_conversion_fn: ConvConversionFn,
                                                 conv_op_factory_fn: ConvOpFactory) -> list[tflite_model.Operator]:
    ...
    splitter = _InputTensorsSplitter(conversion_result.conv_input_tensor, conversion_result.conv_weight_tensor,
                                     conversion_result.conv_bias_tensor, conv_params.groups, builder)
    combiner = _OutputTensorsCombiner(conversion_result.conv_output_tensor, conv_params.groups, builder)
    ...
    for i in range(conv_params.groups):
        ...
        conv_ops_list = conv_op_factory_fn(conv_params, input_tensor, weight_tensor, bias_tensor, output_tensor,
                                           builder, conv_builtin_options)

        conv_ops.extend(conv_ops_list.flatten())

    return (conversion_result.ops_list.pre_ops +  # `Pad` operator
            splitter.get_ops() + conv_ops + combiner.get_ops() +  # Split, Conv2D, Concatenate ops
            conversion_result.ops_list.post_ops)  # Currently not used
