...
from executorch.backends.nxp.backend.ir.converter.node_converters.shared.conv_utils import ConvConversionResult, \
    ConvParameters, get_node_tensor_params
...
from executorch.backends.nxp.backend.ir.tflite_generator.builtin_options import conv_2d_options, \
    depthwise_conv_2d_options, reshape_options, transpose_conv_options
...
class ConvolutionConverter(NodeConverter):
    ...
    def _is_supported_on_target(
        ...
        match target:
            case Target.RT700:
                ...
                weights = node.args[1]
                conv_params = ConvParameters(*ConvolutionConverter._get_convolution_arguments(node))
                ...
                if conv_params.transposed:
                    ...
                    if (
                        ...
                        or (conv_params.padding[1] != 0 and node_t_params["inp_height"] != 1) # Slice added by explicit padding
                        or conv_params.stride[0] != 1
                        or ((conv_params.stride[1] != node_t_params["kernel_width"] / 2 or node_t_params["out_height"] != 1)
                    ...
                    pass
                elif conv_utils.group_conv_convertible_as_depthwise(node, conv_params.groups):  # Depthwise convolution.
                    # Only supported if the weights are static, because TFLite `DepthwiseConv2D` uses permuted
                    #  weights. In case the weights are dynamic, a Transpose operator would have to be added, which
                    #  is not supported on Neutron.
                    if not node_is_effectively_static_tensor(weights, parameters_mapping):
                        return False
                elif conv_utils.group_conv_convertible_into_multiple_convolutions(node, conv_params.groups):  # Separable conv.
    ...
    def _is_supported_in_IR(
        ...
        if is_transposed and conv_utils.group_conv_convertible_as_depthwise(node, groups):
    ...
    @staticmethod
    def _get_convolution_arguments(conv_node: Node
        ...
        _, _, _, stride, padding, dilation, transposed, out_padding, groups = conv_node.args
        return list(stride), list(padding), list(dilation), transposed, list(out_padding), groups

    def _convert_1d_conv(self, t_op: tflite_model.Operator, conv_params: ConvParameters) -> list[tflite_model.Operator]:
    ...
    def _convert_unpadded_2D(self, t_op: tflite_model.Operator, conv_params: ConvParameters
        ...
        if (b := try_get_input(t_op, 2)) is None:
            ...
            if w.type == TensorType.FLOAT32:
                ...
                # Should never happen.
                raise NotImplementedError(f"Convolution node with unsupported weight type: {w.type}")

            b = self.builder.create_zeros_tensor([output_channels], "zero_bias", bias_type, False)
            ...
            if w.type in [TensorType.INT8, TensorType.UINT8]:
                ...
                set_quantization_parameters_to_tensor(b, bias_scale, bias_zero_point, quantized_dimension=0)
    ...
    def _convert_transpose_conv(self, t_op: tflite_model.Operator, conv_params: ConvParameters
                             ) -> conv_utils.ConvConversionResult:
        ...
        if (b := try_get_input(t_op, 2)) is None:
            ...
            b = self.builder.create_zeros_tensor([output_channels], "zero_bias", bias_type, True)
        ...
        o = self.builder.create_tensor_for_data(output_shape_tensor_data, "output_shape")

        # Assign the operator its TFLite inputs and outputs
        t_op.tmp_inputs = [o, w, x, b]
        t_op.tmp_outputs = [y]
        conversion_result = ConvConversionResult(x, w, b, y, o)
        t_op.builtin_options.padding, explicit_padding = aten_translator.convert_padding(conv_params.padding)
    ...
    def _convert_2d_conv(self, t_op: tflite_model.Operator, conv_params: ConvParameters) -> list[tflite_model.Operator]:
        if conv_params.transposed:
            t_op.builtin_options = transpose_conv_options.TransposeConv()
            if conv_utils.group_conv_convertible_into_multiple_convolutions(t_op, conv_params.groups):
            ...
            if conv_utils.group_conv_convertible_as_depthwise(t_op, conv_params.groups):  # Convert to `DepthwiseConv2D`.
                ...
                if explicit_padding is not None:
                    ...
                    conversion_result.ops_list.add_pre(self.builder.create_pad_operator_before(t_op, 0, explicit_padding))
            ...
            elif conv_utils.group_conv_convertible_into_multiple_convolutions(t_op, conv_params.groups): # Convert to separated `Conv2D`.
                t_op.builtin_options = conv_2d_options.Conv2D()

                return conv_utils.create_separated_convolutions_based_on_group(
                    t_op, conv_params, self.builder, self._convert_unpadded_2D, conv_utils.conv_op_factory)
    ...
    def convert(self, node: Node):
        ...
        stride, padding, dilation, transposed, out_padding, groups = self._get_convolution_arguments(node)

        t_op = self._create_tflite_op_with_io_tensors(node)
        conv_params = ConvParameters(stride, padding, dilation, transposed, out_padding, groups)
        ...
        if rank == 3:  # Conv1D
            ...
            raise NotImplementedError(f'{rank - 2}D convolution is not supported.')  # Should never get here.
