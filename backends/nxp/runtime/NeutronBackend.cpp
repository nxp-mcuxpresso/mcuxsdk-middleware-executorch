 ...
 public:
  ...
  Result<DelegateHandle*> init(
    ...
    case 1:
      ...
      break;
    default:
      ET_LOG(Error, "Unknown payload version %d. Please update the backend", payloadVersion);
      return Error::InvalidProgram;
    }
  ...
  Error execute(
    ...
    // There are 3 extra entries: scratch, profile and debug. The scratch pointer was allocated implicitly in the previous versions.
    cfg->dcfg.outputs =
        static_cast<void**>(context.allocate((cfg->numOutputs + 3) * sizeof(void*)));
    cfg->dcfg.outputs[cfg->numOutputs] =
        static_cast<void*>(context.allocate(cfg->scratchSize, 16));
    cfg->dcfg.outputs[cfg->numOutputs + 1] = static_cast<void*>(context.allocate(cfg->profileSize, 16));
    cfg->dcfg.outputs[cfg->numOutputs + 2] = static_cast<void*>(context.allocate(cfg->debugSize, 16));
