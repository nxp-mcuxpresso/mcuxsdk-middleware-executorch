...
class Conv2dPermuteModule(torch.nn.Module):
    def __init__(self, in_channels: int, new_dims: tuple[int, ...]):
        ...
        self.conv = Conv2dModule(
            in_channels=in_channels, out_channels=in_channels, stride=1, kernel_size=2
        )
...
class TestPermuteCopyConversion(kgb.SpyAgency, unittest.TestCase):
    ...
    @parameterized.expand(
        [
            ["To channel first permutation", (1, 16, 9, 9), (0, 3, 1, 2)],
            ["To channel last permutation", (1, 16, 9, 9), (0, 2, 3, 1)],
        ]
    )
