...
class TestTanhConverter(unittest.TestCase):
    ...
    def test_conv_tanh(self, _: str, inplace: bool, input_shape: tuple[int] = (1, 3, 112, 112)):
        with kgb.spy_on(EdgeProgramToIRConverter.convert_program, call_original=True) as converter_spy:
            if inplace:
                model = Conv2dWithActivation(activation=torch.tanh_, in_channels=input_shape[1])
            else:
                model = Conv2dWithActivation(activation=torch.tanh, in_channels=input_shape[1])
            ...
            lowered_module_graph = quantized_program.graph_module.lowered_module_0.original_module.graph
            tanh_ops = [exir_ops.edge.aten.tanh.default, exir_ops.edge.aten.tanh_.default]
